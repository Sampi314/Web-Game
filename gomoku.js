...
function getDifficultySettings() {
    switch (botLevel) {
        case 'easy':
            return {
                searchRadius: 1,
                candidateCount: 8,          // Reduced from 10
                randomness: 0.6,             // Increased from 0.4
                mistakeChance: 0.5,          // Increased from 0.3
                checkMultiThreats: false,
                lookAhead: 0,
                missObviousThreat: 0.25      // NEW: 25% chance to miss obvious threats
            };
        case 'medium':
            return {
                searchRadius: 2,
...
// Opening book - strategic first moves based on game rules
function getOpeningMove() {
    const center = Math.floor(BOARD_SIZE / 2);

    // Different openings for blocked vs non-blocked rules
    const openings = {
        blocked: [
            { row: center, col: center, name: 'Direct' },        // Center - classic
            { row: center - 1, col: center - 1, name: 'Indirect' },  // Off-center diagonal
            { row: center + 1, col: center - 1, name: 'Soosyrv' },   // Soosyrv opening
            { row: center - 1, col: center, name: 'High' },        // One above center
            { row: center, col: center + 1, name: 'Right' },       // One right of center
        ],
        freestyle: [
            { row: center, col: center, name: 'Center' },        // Dominant center
            { row: center - 2, col: center, name: 'Aggressive' },  // Aggressive high
            { row: center, col: center - 2, name: 'Wide' },        // Wide opening
            { row: center - 1, col: center - 1, name: 'Diagonal' }, // Diagonal approach
        ]
    };

    const pool = blockedRule ? openings.blocked : openings.freestyle;

    // Expert and Hard: prefer center/direct openings
    // Medium: balanced selection
    // Easy: random selection
    if (botLevel === 'expert' || botLevel === 'hard') {
...
function findBestMove() {
    ...
    // 2. Block opponent's winning move (Easy bot might miss this sometimes)
    const blockMove = findWinningMove(humanPlayer);
    if (blockMove) {
        // Easy bot has a chance to miss obvious threats
        if (settings.missObviousThreat && Math.random() < settings.missObviousThreat) {
            // Skip blocking and continue to regular evaluation
        } else {
            return blockMove;
        }
    }
...
function findBestMoveByEvaluation(botPlayer, humanPlayer) {
    ...
    // If no stones on board, use opening book
...
function evaluateMove(row, col, botPlayer, humanPlayer) {
    ...
    if (blockedRule) {
        // When blocked rule is ON, heavily prioritize open-ended patterns
        if (offensiveScore >= PATTERNS.OPEN_THREE) {
            score *= 1.3; // Bonus for open patterns
        }
        if (defensiveScore >= PATTERNS.OPEN_THREE) {
            score *= 1.2; // Important to block open threats
        }
    } else {
        // When blocked rule is OFF (freestyle), be more aggressive
        if (offensiveScore >= PATTERNS.FOUR) {
            score *= 1.4; // Push for quick wins
        }
    }
